 ...
 */
function gadnews_rewrite_comment_item( $_comment, $args, $depth ) {
	global $comment;

	$_comment->gadnews_comment_list_args = $args;
	$comment = $_comment;
 ...
 */
function gadnews_comment_author_avatar( $args = array() ) {
	global $comment;

	if ( ! empty( $comment->gadnews_comment_list_args['avatar_size'] ) ) {
		$size = $comment->gadnews_comment_list_args['avatar_size'];
	}
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'gadnews_comment_author_avatar', get_avatar( $comment, $size ), $args );
}
 ...
 */
function gadnews_get_comment_author_link( $args = array() ) {
	/**
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'gadnews_get_comment_author_link', sprintf( '<b class="fn">%s</b>', get_comment_author_link() ), $args );
}
 ...
 */
function gadnews_get_comment_date( $args = array() ) {
	 ...
	 * @param array  $args   Arguments.
	 */
	return apply_filters( 'gadnews_get_comment_date', sprintf( '<a href="%3$s" class="comment-date"><time datetime="%1$s" class="comment-date__time">%2$s</time></a>', get_comment_time( 'c' ), get_comment_date( $format ), get_comment_link() ), $args );
}

/**
 * Get the html-formatted comment human time diff.
 *
 * @return string.
 */
function gadnews_get_human_time_diff(){
	$time_diff = human_time_diff( get_comment_time('U'), current_time('timestamp') );
	$time_diff = sprintf(
		esc_html__( '%s ago', 'gadnews' ),
		$time_diff
	);
	$format_time_diff = '<span class="comment-human-time-diff">'.$time_diff.'</span>';
	return $format_time_diff;
}
 ...
 */
function gadnews_get_comment_text( $args = array() ) {
	 ...
	 * @param array  $args         Arguments.
	 */
	return apply_filters( 'gadnews_get_comment_text', $comment_text, $args );
}
 ...
 */
function gadnews_get_comment_reply_link( $args = array() ) {
	global $comment_depth;
	 ...
	 * @param array  $args  Arguments.
	 */
	return apply_filters( 'gadnews_get_comment_reply_link', $reply, $args );
}
 ...
 */
function gadnews_get_comment_link_edit( $args = array() ) {
	global $comment;

	$text = __( 'Edit', 'gadnews' );
	 ...
	 * @param array  $args       Arguments.
	 */
	return apply_filters( 'gadnews_get_comment_link_edit', $link, $comment->comment_ID, $args );
}